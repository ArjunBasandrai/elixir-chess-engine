...
namespace elixir::search {
    ...
    int Searcher::qsearch(ThreadData &td, int alpha, int beta, PVariation &pv, SearchStack *ss) {
        ...
        eval = history.correction_history.correct_static_eval(eval, board.get_side_to_move(),
                                                              board.get_pawn_hash());
        /*
    ...
    int Searcher::negamax(ThreadData &td, int alpha, int beta, int depth, PVariation &pv,
        ...
        */
        ...
        if (! ss->excluded_move) {
            ...
            ss->static_eval =
                history.correction_history.correct_static_eval(ss->static_eval,
        ...
        */
        ...
        if (! ss->excluded_move) {
            ...
            if (! in_check && (! best_move || ! best_move.is_capture()) &&
                ! (best_score >= beta && best_score <= ss->static_eval) &&
                ! (! best_move && best_score >= ss->static_eval)) {
                const int bonus = std::clamp((best_score - ss->static_eval) * depth / 8,
                                             -pawn_history_size / 4, pawn_history_size / 4);
                history.correction_history.update_correction_history(bonus,
                                                                     board.get_side_to_move(),
                                                                     board.get_pawn_hash());
            }
        }

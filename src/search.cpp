...
namespace elixir::search {
    ...
    int Searcher::negamax(ThreadData &td, int alpha, int beta, int depth, PVariation &pv,
        ...
        */
        ...
        if (! in_check && (! best_move.is_capture() || best_move == move::NO_MOVE) &&
            ! (best_score >= beta && best_score <= ss->eval) &&
            ! (best_move == move::NO_MOVE && best_score >= ss->eval)) {
                history.update_corrhist(board.get_side_to_move(), board.get_pawn_hash(), depth, best_score, ss->eval);
        }

...
namespace elixir::search {
    ...
    int Searcher::negamax(ThreadData &td, int alpha, int beta, int depth, PVariation &pv,
        ...
        */
        ...
        if (! pv_node && ! in_check && depth > 3 && std::abs(beta) < MATE_FOUND &&
            ! (tt_hit && result.depth >= depth - 3 && result.score < probcut_beta)) {
            ...
            while ((move = mp.next_move())) {
                ...
                score = -qsearch(td, -probcut_beta, -probcut_beta + 1, local_pv, ss + 1);

                if (score >= probcut_beta) {
                    score = -negamax(td, -probcut_beta, -probcut_beta + 1, depth - 4, local_pv, ss + 1, ! cutnode);
                }

                board.unmake_move(move, true);

                if (score >= probcut_beta) {
                    tt->store_tt(board.get_hash_key(), score, move, depth - 3, ss->ply, TT_ALPHA, pv);
                    return score;
                }

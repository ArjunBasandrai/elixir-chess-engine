        ...
        */
        ...
        if (!ss->excluded_move) {
            if (in_check)
                ...
                const bool should_use_tt_score = (tt_flag == TT_EXACT
						|| tt_flag == TT_ALPHA && result.score < ss->static_eval
						|| tt_flag == TT_BETA  && result.score > ss->static_eval);
                ss->static_eval = board.evaluate();
                ss->eval = (tt_hit && should_use_tt_score) ? result.score : ss->static_eval;
            }
        }
        ...
        if (! pv_node && ! in_check && !ss->excluded_move) {
            ...
            */
            if (depth <= RFP_DEPTH && ss->eval - futility_margin(depth, improving, cutnode, tt_hit) >= beta && beta >= -MATE_FOUND && ss->eval <= MATE_FOUND) {
            ...
            */
            if (depth >= NMP_DEPTH && (ss - 1)->move && ss->eval >= beta && board.has_non_pawn_material()) {
                int R = NMP_BASE_REDUCTION + depth / NMP_DIVISOR + std::min((ss->eval - beta) / 200, 6) + std::min(board.get_phase(), 24) / 8;
                R     = std::min(R, depth);
